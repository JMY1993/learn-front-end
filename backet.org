...
** for...in循环会跳过Symbol类型的键值对，但!!!Object.assign会复制Symbol类型的键值对
...
* try...catch仅对运行时的错误有效，无法处理(parse-time)解析时间错误
* try...catch同步工作，对包裹的scheduled代码无法处理
* error对象：name，message，stack
* throw抛出错误，会被后面的catch捕获
* catch块中可以通过包裹if else语句，重新抛出错误
* finally无论如何会被执行，即使try...catch中有return语句，finally也会执行
** !!!一个finally的优点的例子
对比以下两种例子：
#+begin_src js
try {
...
#+end_src
有什么区别呢？
区别在于：如果try...catch语句被函数包裹，finally语句保证了cleanup的执行
finally 子句适用于 try..catch 的 任何 出口。这包括显式的 return，rethrow等行为。
这种情况下，如果没有把cleanup code包裹在finally中，cleanup是不会执行的
* try...catch...finally中生命的变量是局部变量，只在对应的块中可见
* 全局error的作用通常是将错误信息发送给开发者
* !!!class语法的constructor有name属性
应用：简化error类声明时对于name字段的赋值
...
以下代码将syntax error和validation error（低级别错误）包装进了ReadError中，
#+begin_src js
...
#+begin_src js
  ...
  promise.then(
    successFn(result); // result是Promise的resolve的参数
    failedFn(err);  // err是Promise的reject的参数
  )
    .catch(
      failedFn(err);
    )
...
#+end_src
!!! .then()返回的promise的状态是resolved
#+begin_src js
let promise = new Promise((resolve,reject)=>reject("err")); //promise的state是rejected
let promise1 = promise.then(null, str => {cosole.log(str);return str;});  //promise1的state是resolved
let promise2 = promise1.catch(console.log);                 //控制台输出undefined
//如果then的?onrejected没有rethrow error，下面的catch也接不到这个err,如果再链接一个then，新的then的?onfullfilled会接到这个返回值
//如果then的?onrejected rethrow了一个error，下面的catch会接到这个err
#+end_src
!!! 它的参数 resolve 和 reject 是由 JavaScript 自身提供的回调。我们的代码仅在 executor 的内部。换句话说，resolve和reject不能更改，这个名字是固定的。
!!! promise的then/catch/finally执行与否只与promise对象的state有关，如果state是settled（无论是resolved还是rejected），then都会立即执行。
如果为一个已经settled的promise添加上述消费者方法，这些方法中的代码都会立即执行。
* promise链：promise的then会返回一个新的Promise对象。then回调函数的return值会成为新的Promise对象的settle值。
** 传递result或者error，finally会直接传递，它的回调不需要参数
** finally 并不是意味着要处理 promise 的结果。所以它将结果传递了下去。（也不需要result）
...
then的回调函数中执行的实际上是一个同步过程。意味着，在then的回调函数执行时，其中的setTimeout跟普通同步过程是一样的。setTimeout后面的代码不会等待setTimeout执行完成才执行。(*)行代码后面的return在 ~string+='baz'~ 之前就发生了。

如果在then语句中有setTimeout（计划调用）类过程，要新建并返回一个promise，在这个新的promise中做计划调用。（**）行代码的做法，使得后续的then等待其resolve后才执行。
...
返回的promise的resolve值是下一个then的回调的参数。
* !!!fetch有两段
- 第一段返回一个response（promise对象），当远程服务器返回 header（是在 全部响应加载完成前）时
- 在下一个then中调用response.text()方法=>返回一个promise对象，用json字符串resolve
或response.json()方法=>返回一个promise对象，是JSON.parse()后的对象
** !!!一个加载github头像的例子
#+begin_src js
// 发送一个对 user.json 的请求
...
#+end_src
这段代码的问题在上文中setTimeout和promise的探讨中说过了，（**）中的代码有一个异步过程，如果想要对加载头像后的状态进行处理实际上是做不到的。应该做如下修改（*）行的修改：
#+begin_src js
  fetch('/article/promise-chaining/user.json')
    .then(response=>response.json())
    .then(user=>fetch(`https://api.github.com/users/${user.name}`))
...
//上面两种写法有区别。第一种写法如果有error，会被catch处理。而第二种不会。因为error是沿着链传递的，第二种没有链
#+end_src
* promise的隐式try...catch
下面两段代码工作上完全相同
#+begin_src js
...
#+end_src
可以理解为throw new Error周围有一个try...catch，并将其变为reject(new Error);
然后reject的err被后面链接的catch捕获
...
#+end_src
* then/catch中也可以throw error，并且被最近的catch捕获
...
#+end_src
* Promise类的静态方法all/allSettled/race/resolve/reject
** 批量执行异步任务Promise.all()/Promise.allSettled()
语法：
接受一个promise对象构成的的数组，返回一个promise，其resolve是每个promise对象resolve结果组成的数组
...
*** !!!Promise.all()/Promise.allSettled()可以接受一个iterable对象，如果任意一个元素不是promise对象，它将被“原样”返回（as is）
*** Promise.allSettled()及其polyfill
...
因为这个方法是新加入的特性，不被所有环境支持，如果没有这个方法，可以polyfill一个：
#+begin_src js
  //这是我的代码
...
//!!!p => Promise.resolve(p) 将输入值转换为 promise（以防传递了 non-promise)，这一点很重要，因为Promise.all/Promise.allSettled支持数组中有非promise的元素。
#+end_src
** Promise.race([...promise...])
- 执行数组中所有的promise，只要有一个完成了，所有进一步的 result/error 都会被忽略。
- resolve值是一个值，不是数组
...
#+begin_src js
  ...
  //如果没有在catch中return一个new Promise：
  let promise = Promise.reject(new Error("Promise Failed!"));
  ...
  //也是工作的，因为catch在promise之后就被放进了微任务队列，虽然setTimeout代码处理发生在catch结束之后，但是catch结束后只要没有rethrow，就返回了一个resolve状态的promise，因为不会触发'unhandledrejection'
#+end_src
!!!unhandledrejection：引擎会检查 promise，如果 promise 中的任意一个出现 “rejected” 状态，unhandledrejection 事件就会被触发。
* Promisification
** 定义一个双参回调的promisify
** 定义一个多参回调的promisify
* generator返回一个具有.next()方法的对象，这个对象是iterable的
语法：
#+begin_src js
...
** 如果一个值是由return（而不是yield）产生的，这个值的done属性就是true，反之false
** generator对象迭代的时候，done:false属性对应的value会被忽略
* Generator 组合（composition）是将一个 generator 流插入到另一个 generator 流的自然的方式。它不需要使用额外的内存来存储中间结果。
下面的代码定义了两个generator，第一个是产生数字序列的
第二个generator根据数字序列产生string

可以观察到有三个for循环，产生不同区间的数字
这三个for循环每个都是可以用另一generator产生的
...
代码运行分析：
共调用了六次next。
r1: 1- yield 1 停在（1）等待给tag值；
    2- 返回结果1
    此时i=1;
r2: 1- 给tag值 first call
    2- 执行console.log，这里输出的是firstcall，但i没变，还是1
    3- 第一个循环结束，i=2;
    4- yield 2，停在（1）等待给tag值；
...
r6

可以看到一个问题：首次循环中yield下面的内容是在第二次yield时执行的
也就是说，每次循环中的其它代码是在其后一次next中执行的

如果我们把yield赋值的等号看成一个分界线：
let [1.下次next赋值] = yield [2.本次的结果]；
3.其它代码1
4.其它代码2
...
上面的error被抛到（1）行的yield，然后被下面的catch接住。如果没有接住，（2）行作为调用代码就会出现异常（generator中的error掉出到调用代码中）。

如果error掉出，可以在掉出代码中处理：
...
因为spread语法需要Symbol.iterator，但是上面例子定义的range只有Symbol.asyncIterator
** !!!async generator
语法：
1. async function* generateSequence(start, end) {}
2. async generator定义函数内部可以使用await；
3. 遍历async generator时，使用 ~for await...of~ 结构
4. !!!async generator的next方法返回的是promise，意味着调用 ~await asyncGenerator.next()~ 时，前面要加 ~await~ 关键字（或者也可以用then）
** !!!async iterable
*** 同步/异步 iterable 的generator定义
1. 普通generator定义可迭代对象的Symbol.iterator
注意[Symbol.iterator]前面的星号
#+begin_src js
let range = {
  from: 1,
  to: 5,
...
#+end_src
2. 异步（async）generator定义可迭代对象
首先使用Symbol.asyncIterator
其次注意Symbol.asyncIterator前面的星号
!!!星号代表是一个generator，asyncIterator代表是一个异步迭代器方法
#+begin_src js
let range = {
  from: 1,
  to: 5,
...
#+end_src
** 总结异步迭代
迭代有iterable对西那个和generator两种
* iterable和async iterable
|                        | iterable                     | async iterable                                 |
|------------------------+------------------------------+------------------------------------------------|
| 提供iterator的对象方法 | Symbol.iterator              | Symbol.asyncIterator                           |
| next() 的返沪值        | {value:..., done:true/false} | resolve成{value:..., done:true/false}的promise |

* generator和async generator
|                | generator                    | async generator                                |
|----------------+------------------------------+------------------------------------------------|
| 声明方式       | function*                    | async function*                                |
| next()的返回值 | {value:..., done:true/false} | resolve成{value:..., done:true/false}的promise |
* 模块基础知识
** 语言级的模块系统从ES6开始支持
之前版本的模块由：
AMD
...
UMD
等库提供
** 模块的载体：一个文件就是一个模块，一个脚本就是一个模块
** 关键字
*** export标记了可以从外部访问的关键字和变量
*** import允许从其它模块导入功能
一个例子：
#+begin_src js
  // 📁 sayHi.js
...
** 浏览器不允许裸模块，即没有任何路径的模块
** 兼容性：使用 ~<script nomodule>~ 标签，提供后备
** 一个模块就是一个文件。浏览器需要使用 <script type="module" src=""> 以使 import/export 可以工作。模块相较于常规脚本有几点差别：
- 默认是延迟解析的（deferred）。
- Async 可用于内联脚本。
- 要从另一个源（域/协议/端口）加载外部脚本，需要 CORS header。
- 重复的外部脚本会被忽略(具有相同 src 的外部脚本仅运行一次)
#+begin_src html
...
#+end_src
** 动态导入import(modulePath)，返回一个resolve为所有导出对象的的obj。（modulePath是模块路径的string）
*** 动态导入可以在代码中的任意位置动态地使用（静态导入不能直接做赋值等操作）
*** 动态导入在常规脚本中工作时，它们不需要 script type="module"
*** 尽管 import() 看起来像一个函数调用，但它只是一种特殊语法，只是恰好使用了括号（类似于 super()）
*** !!!因此不能将 ~import~ 复制到一个变量中，或者对其使用 call/apply。因为它不是一个函数。
* Proxy
** 保护内部属性
** Proxy与继承
** 内建对象的内部插槽(internal slot)
*** 私有字段
** 可撤销的proxy
** 几个例子：
*** !!!Proxy方法的参数中，prop是字符串：
#+begin_src js
let array = [1, 2, 3];
